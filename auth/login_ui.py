"""The Vermo login/signup page, shared by both auth backends.

auth/local_auth.py (SQLite-era password auth) and auth/supabase_auth.py
(Supabase Auth, used when VERMO_BACKEND=postgres) render the identical page —
only the credential callables differ. Keeping the page here means the branding
and the finicky Streamlit CSS live in exactly one place.
"""
...
def render_login_page(
    *,
    login: Callable[[str, str], tuple[bool, str]],
    register: Callable[[str, str, str], tuple[bool, str]],
    reset_password: Optional[Callable[[str, str], tuple[bool, str]]] = None,
) -> None:
    """Render the login/signup page and st.stop().

    login(email, password) and register(email, password, display_name) return
    (ok, message). reset_password(email, new_password) enables the inline
    "Forgot password?" flow; pass None to hide it (Supabase Auth's email-based
    recovery replaces it — see MULTI_USER_ROADMAP Stage 4).
    """
    ...
    with right:
        ...
        with tab_login:
            if reset_password is not None and st.session_state.get("show_password_reset"):
                st.markdown(
                    """
                ...
                with st.form("login_form"):
                    ...
                    if st.form_submit_button("Log in", type="primary"):
                        ...
                        if ok:
                            ...
                            st.error(message)
                if reset_password is not None and st.button("Forgot password?"):
