...
| `market_data.py` | ~140 | Single source of truth for the curated ticker maps (Yahoo symbols, cap buckets, barbell roles), classification functions, and FX/quote fetchers. No DB/framework imports. |
| `portfolio_core.py` | ~180 | Shared portfolio domain logic: `refresh_prices` orchestration, FX settings, daily snapshots. Callers inject `connect_fn`. This makes the Streamlit app self-contained (no localhost FastAPI hop) for hosted deployments. |
| `budget_db.py` | ~150 | Extracted budget-expense data layer (`add_expense`, `expense_exists` duplicate guard). Exists so this logic is importable and unit-testable — `streamlit_app.py` executes login/rendering at import time and cannot be imported by tests. **This is the pattern to follow when extracting more logic** (see DEVELOPMENT.md). |
